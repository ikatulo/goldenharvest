...
<?php
	...
	$team = get_post($uri[4]);
	$teamCustomer = get_field('customers', $team->ID);
	$teamMembers = get_field('assigned_to', $team->ID);

	$arrMemberIds = array();

	foreach($teamMembers as $member){
		array_push($arrMemberIds, $member['ID']);
	}
    ...
    <form id="addTeam" role="form" class="form-horizontal inner-form">
    	<div class="form-section">
	    	...
	    	<div class="form-group" id="teamName">
			    ...
			    <div class="col-sm-10">
			      <input type="text" class="form-control" id="teamName" name="teamName" placeholder="Team Name" value="<?php echo $team->post_title;?>">
			    </div>
		  	</div>
			    ...
			    <div class="col-sm-10">			      
			      <select id="customer" name="customer" class="standard-dropdown" data-placeholder="Select Customer">
			      	...
			      	<?php while($customers->have_posts()): $customers->the_post();?>
			      		<?php $selected = $teamCustomer->ID == get_the_ID() ? 'selected="selected"' : '';?>
			    ...
			    <div class="col-sm-10">			      
			      <select id="members" name="members[]" class="multiple-dropdown" data-placeholder="Select Team Members" multiple>
			      	...
			      	<?php foreach($users->results as $user):?>
			      		<?php $selected = in_array($user->ID, $arrMemberIds) ?  'selected="selected"' : '';?>
			      		<option value="<?php echo $user->ID;?>" <?php echo $selected;?>><?php echo $user->display_name.' ('.$roles[$user->roles[0]].')';?></option>
			      	<?php endforeach;?>
			      </select>
